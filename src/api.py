...
class FlaskInterface:
    ...
    def setup_routes(self):
            ...
            try:
                ...
                if self.event_loop.is_running():
                    ...
                    future = asyncio.run_coroutine_threadsafe(
                        self.node.submit_evalution(url, token),
                        self.event_loop
                    )
                    eval_id = future.result()
                else:
                    # If loop is not running, use a new temporary event loop
                    temp_loop = asyncio.new_event_loop()
                    eval_id = temp_loop.run_until_complete(self.node.submit_evalution(url, token))
                    temp_loop.close()
        ...
        def get_network():
            # basic info just for now
            return jsonify({
                "node_address": f"{self.node.address[0]}:{self.node.address[1]}",
                "network_type": "in develpment :)"
            }), 200
